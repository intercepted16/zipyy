<script lang="ts">
  ...
  import UrlModal from "../lib/components/UrlModal.svelte";
  import ViewAllModal from "../lib/components/ViewAllModal.svelte";
  let isFocused: boolean;
  let isInvalid: boolean = true;

  const validateUrl = async () => {
    let val = String(get(url))
      .replace(/^https?:\/\//, "")
      .toLowerCase();
    url.set(val);
    ...
    shortened.set(data["shortened"]);
    urlModal.set(true);
  };
  const isValidUrl = () => {
    let pattern = /^[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,4}/g;
    return pattern.test(get(url).replace(/^(https?:\/\/)?(www\.)?/, ""));
  };
</script>
  ...
  <div class="relative px-6 lg:px-8">
    <section class="bg-white dark:bg-gray-900">
      ...
      >
        ...
        >
          The <span class="text-green-500 excludedText">best</span> URL shortener.
        </h1>
        <p
          class="mb-8 text-lg font-normal text-gray-500 lg:text-xl sm:px-16 xl:px-48 dark:text-gray-400"
        >
          A no-nonsense, free, private URL shortener you can trust. No accounts,
          no limits, no bloat! Built using
          <strong class="font-medium text-slate-800 whitespace-nowrap"
            >Flask</strong
          >,
          <strong class="font-medium text-slate-800 whitespace-nowrap"
            >Tailwind CSS</strong
        ...
        >
          <span class="font-semibold text-gray-400 uppercase">Really?</span>
          <div
            class="flex flex-wrap items-center justify-center mt-8 text-gray-500 sm:justify-between"
          >
            <a
              href="#"
              class="mb-5 mr-5 opacity-50 flex-center lg:mb-0 hover:text-gray-800 dark:hover:text-gray-400 hover:opacity-100 text-nowrap"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                ...
                ></svg
              >
              <span class="ml-2 text-xl font-semibold">Limited Bitly</span>
            </a>
            <a
              href="#"
    ...
    <div
      class="w-full mx-auto rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:border-gray-700 dark:text-white dark:bg-gray-800"
    >
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8 dark:text-white">
        ...
        <div>
          <form
            class="space-y-4 md:space-y-6 dark:text-white group"
            id="urlForm"
            novalidate
          >
            <div class="flex flex-col sm:flex-row dark:text-white">
              <input
                type="text"
                name="url"
                id="urlInput"
                ...
                on:input={(e) => {
                  ...
                  isInvalid =
                    !e.target.checkValidity() || !isValidUrl(get(url));
                  if (!isValidUrl(get(url)))
                    e.target.setCustomValidity("Invalid URL.");
                }}
                class:!border-red-500={isInvalid && !isFocused && get(url)}
                placeholder="Paste your URL here"
                class="bg-gray-50 border-2 border-gray-300 text-gray-900 sm:text-sm focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:text-white mb-2 sm:mb-0 rounded-lg sm:rounded-none"
                required
              />
              <Button
                type="button"
                id="shortenBtn"
                class="w-full border-2 border-blue-500 sm:rounded-none sm:!rounded-r-md text-white focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium text-sm px-5 py-2.5 text-center dark:focus:ring-primary-800 group-invalid:pointer-events-none dark:text-white "
            ...
            <span
              class:!block={isInvalid && !isFocused && get(url)}
              class="!mt-2 hidden text-sm text-red-500 excludedText"
            >
    ...
    {#if $entries.length > 0}
      <div
        class="max-w-md p-4 mx-auto mt-8 mb-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700"
      >
        <div class="flex items-center justify-between">
          ...
          <Button
            on:click={() => {
              viewAllModal.set(true);
            }}
            class="p-0 text-sm font-medium !ring-0 hover:underline"
            color="none"
          >
            View all
          </Button>
