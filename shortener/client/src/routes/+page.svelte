<script lang="ts">
  ...
  $: if ($page.url.hash.slice(1).startsWith("error=")) {
    const verificationError: ModalSettings = {
      ...
      body: "We couldn't verify your account because the email link is expired or invalid.",
      buttonTextCancel: "Okay",
    };
    modalStore.trigger(verificationError);
  }
  ...
  const validateUrl = async () => {
    ...
    shortened.set(response.data);
    modalStore.trigger(urlModal);
    // urlModal.set(true);
  };
  const isValidUrl = () => {
...
<div
  class="flex flex-col items-center max-w-screen-xl py-8 mx-auto text-center lg:py-16"
>
  <a
    href="#"
  ...
  <div
    class="container flex flex-col items-center justify-center p-4 mx-auto text-center"
  >
    <div class="relative z-[1] space-y-6 pb-24">
      ...
      >
        ...
        <strong
          class="font-medium text-slate-800 dark:text-white whitespace-nowrap"
          >Flask</strong
        >,
        <strong
          class="font-medium text-slate-800 dark:text-white whitespace-nowrap"
      ...
      <div class="flex flex-wrap gap-3 md:space-x-3 md:block">
        <a
          ...
          ><i class="mr-2 bi bi-github"></i>View on Github</a
        >
        <button class="w-full btn md:btn-lg md:w-fit variant-filled-primary"
          >Get started</button
        >
      </div>
    </div>
  ...
  >
    <span class="font-semibold text-gray-400 uppercase excludedText"
      >Really?</span
    >
    <div
    ...
    >
      <a
        ...
        class="mb-5 mr-5 text-black opacity-50 dark:text-white flex-center lg:mb-0 hover:opacity-100 text-nowrap"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          ...
          ></svg
        >
        <span class="ml-2 text-xl font-semibold">Capped Bitly</span>
      </a>
      <a
        href="#"
...
<div
  class="mx-auto rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:border-gray-700 dark:text-white dark:bg-gray-800"
>
      ...
      >
        <div class="flex flex-col sm:flex-row dark:text-white">
          <input
            ...
            name="url"
            id="urlInput"
            class="mb-2 border-2 sm:rounded-none input sm:mb-0"
            bind:value={$url}
            on:focus={() => {
              isFocused = true;
            }}
            ...
            on:input={(e) => {
              ...
              isInvalid = !e.target.checkValidity() || !isValidUrl(get(url));
          ...
          <button
            type="button"
            id="shortenBtn"
            class="w-full sm:rounded-none active:scale-[100%] btn variant-filled-primary"
